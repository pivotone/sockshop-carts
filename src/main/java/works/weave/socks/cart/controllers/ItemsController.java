...
public class ItemsController {
    ...
    public Result get(@PathVariable String customerId, @PathVariable String itemId) {
        return ResultUtil.success(new FoundItem(() -> (List<Item>) getItems(customerId), () -> new Item(itemId)).get());
    }
    ...
    public Result getItems(@PathVariable String customerId) {
        return ResultUtil.success(((Cart)(cartsController.get(customerId).getData())).contents());
    }
    ...
    public Result addToCart(@PathVariable String customerId, @RequestBody Item item) {
        // If the item does not exist in the cart, create new one in the repository.
        FoundItem foundItem = new FoundItem(() -> ((Cart)(cartsController.get(customerId).getData())).contents(), () -> item);
        if (!foundItem.hasItem()) {
            ...
            return ResultUtil.success(newItem);
        }
    }
    ...
    public Result removeItem(@PathVariable String customerId, @PathVariable String itemId) {
        FoundItem foundItem = new FoundItem(() -> (List<Item>) getItems(customerId), () -> new Item(itemId));
    ...
    public Result updateItem(@PathVariable String customerId, @RequestBody Item item) {
        // Merge old and new items
        ItemResource itemResource = new ItemResource(itemDAO, () -> (Item) get(customerId, item.itemId()).getData());
